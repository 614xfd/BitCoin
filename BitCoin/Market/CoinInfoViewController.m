...
@interface CoinInfoViewController ()

@end

@implementation CoinInfoViewController
...
- (void) request
{
    ...
    if (uid.length) {
        ...
        [[NetworkTool sharedTool] requestWithURLString:@"UserAccount/incomeFromRecord" parameters:d method:@"POST" completed:^(id JSON, NSString *stringData) {
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSString *isIncome = [NSString stringWithFormat:@"%@", [[self.dataArray objectAtIndex:indexPath.row] objectForKey:@"isIncome"]];
    ...
    if ([isIncome isEqualToString:@"0"]) {
        ...
        num.text = [NSString stringWithFormat:@"%@", [[self.dataArray objectAtIndex:indexPath.row] objectForKey:@"to"]];
        amount.text = [NSString stringWithFormat:@"+%@", [[self.dataArray objectAtIndex:indexPath.row] objectForKey:@"amount"]];
    }
