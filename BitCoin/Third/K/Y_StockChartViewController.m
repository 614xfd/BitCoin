...
//    param[@"symbol"] = self.coinStr;
//    param[@"size"] = @"300";
    param[@"time"] = self.type;
    param[@"coin"] = self.coinStr;

//    NSString *string = [NSString stringWithFormat:@"https://bitboss.bitboss.cn/getMarket/obtainKData?coin=%@&time=%@&size=500", self.coinStr, self.type];
//    [NetWorking requestWithApi:string param:nil thenSuccess:^(NSDictionary *responseObject) {
//        if ([responseObject[@"code"] isEqualToString:@"1"]) {
//            NSString *string = responseObject[@"Data"];
//            NSData *JSONData = [string dataUsingEncoding:NSUTF8StringEncoding];
//            NSDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:JSONData options:NSJSONReadingMutableLeaves error:nil];
//            NSArray *array = [NSArray arrayWithArray:responseJSON[@"data"]];
...
//    }];
    ...
    [[NetworkTool sharedTool] requestWithHPURLString:@"https://bitboss.bitboss.cn/getMarket/obtainKData" parameters:param method:@"POST" completed:^(id JSON, NSString *stringData) {
...
//            NSDictionary *responseJSON = [NSJSONSerialization JSONObjectWithData:JSONData options:NSJSONReadingMutableLeaves error:nil];
//            NSArray *array = [NSArray arrayWithArray:responseJSON[@"data"]];
            NSArray *array = [NSArray arrayWithArray:[[JSON objectForKey:@"Data"] objectForKey:@"data"]];
            NSEnumerator *enumerator = [array reverseObjectEnumerator];
            NSArray *arr = [enumerator allObjects];
            Y_KLineGroupModel *groupModel = [Y_KLineGroupModel objectWithArray:arr];
